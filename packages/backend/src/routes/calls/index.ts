...
const callRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/stats', {
    ...
    return {
      ...
      cost_total_30d: parseFloat(String((costRow[0] as any)?.total_cost ?? '0')),
      cost_llm_30d: parseFloat(String((costRow[0] as any)?.cost_llm ?? '0')),
      cost_tts_30d: parseFloat(String((costRow[0] as any)?.cost_tts ?? '0')),
      cost_stt_30d: parseFloat(String((costRow[0] as any)?.cost_stt ?? '0')),
      cost_telephony_30d: parseFloat(String((costRow[0] as any)?.cost_telephony ?? '0')),
  ...
  app.get('/:id/recording', {
    ...
    try {
      // Never send Twilio Basic-Auth to a non-Twilio host (forged/legacy
      // recording_url → credential leak + SSRF).
      if (!isTwilioRecordingUrl(url)) {
        reply.status(502).send({ error: 'Invalid recording URL' });
        return;
      }
  ...
  app.post('/bulk-delete', {
    ...
    const deleted = await callService.bulkDeleteCalls(request.auth.workspaceId, ids);
    return { deleted };
  });
