...
const CAT_COLORS: Record<string, { accent: string; gradient: string }> = {
  ...
  'faq':             { accent: '#8b5cf6', gradient: 'linear-gradient(135deg, rgba(139,92,246,0.12), rgba(139,92,246,0.02))' },
};

const CAT_TITLES: Record<string, { en: string; ru: string }> = {
  'getting-started': { en: 'Getting Started', ru: 'Начало работы' },
  'translator':      { en: 'Using the Translator', ru: 'Работа с переводчиком' },
  'billing':         { en: 'Billing & Balance', ru: 'Баланс и оплата' },
  'settings':        { en: 'Settings', ru: 'Настройки' },
  'faq':             { en: 'FAQ', ru: 'Частые вопросы' },
};

const ART_TITLES: Record<string, { en: string; ru: string }> = {
  'help.art.whatIsTranslator':  { en: 'What is Live Translator', ru: 'Что такое Live Translator' },
  ...
  'help.art.account':           { en: 'Account Settings', ru: 'Настройки аккаунта' },
  'help.art.faq':               { en: 'Frequently Asked Questions', ru: 'Частые вопросы' },
};

/* ── Main Component ─────────────────────────────────────────────────── */
export default function HelpPageClient({ initialLang }: { initialLang?: 'en' | 'ru' | 'es' }) {
              ...
              {selectedArticle && selectedCat ? (
                <AnimatedSection animation="fade-in" key={selectedArticle.titleKey}>
                  ...
                  <div className="rounded-2xl p-6 sm:p-8 help-article-wrapper"
                    style={{ background: 'rgba(26, 32, 44, 0.55)', backdropFilter: 'blur(24px)', border: '0.5px solid rgba(140, 144, 159, 0.12)' }}>
                    <HelpArticle content={selectedArticle.content[lang === 'es' ? 'en' : lang] || selectedArticle.content.en} accentColor={accentColor} />
                  </div>
                </AnimatedSection>
              ) : selectedCat ? (
