    ...
    <div className="bg-[var(--th-card)] rounded-2xl border border-[var(--th-card-border-subtle)] p-5 shadow-[0_1px_3px_var(--th-shadow),0_8px_24px_var(--th-card-glow)]">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {DEPOSIT_PRESETS.map(val => (
          <button
            ...
            className={`px-4 py-3 md:py-2.5 min-h-[44px] rounded-xl text-sm font-semibold border transition-[background,border-color,color] ${
              selectedPreset === val
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 px-4 py-3 sm:py-2.5 rounded-xl border border-[var(--th-border)] bg-transparent text-sm text-[var(--th-text)] placeholder:text-[var(--th-text-muted)] outline-none focus:border-[var(--th-primary)] transition-[border-color]"
        />
        <button
          onClick={handleSubmit}
          disabled={loading || !amount || parseFloat(amount) < 1}
          className="px-6 py-3 sm:py-2.5 min-h-[44px] bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-sm font-semibold rounded-xl shadow-[0_2px_8px_rgba(16,185,129,0.3)] hover:shadow-[0_4px_12px_rgba(16,185,129,0.4)] transition-[box-shadow,transform] disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
        >
