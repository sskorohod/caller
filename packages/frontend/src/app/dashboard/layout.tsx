...
const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 3.75v4.5m0-4.5h-4.5m4.5 0l-6 6m3 2.25c0 8.284-6.716 15-15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25" />
      </svg>
    ),
  },
  {
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    <>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto">
        {[
          ...
          { label: t('nav.sectionOperations') || 'Operations', items: navItems.filter(i => ['/dashboard/calls', '/dashboard/dialer', '/dashboard/missions', '/dashboard/translator'].includes(i.href)) },
  ...
  return (
    ...
    <div className="flex h-screen overflow-hidden bg-[var(--th-page)]">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 bg-[var(--th-overlay)] backdrop-blur-sm z-40 md:hidden" onClick={() => setSidebarOpen(false)} />
      )}

      {/* Sidebar — desktop: static, mobile: slide-in overlay */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-[240px] bg-[var(--th-sidebar)] flex flex-col border-r border-[var(--th-sidebar-border)]
