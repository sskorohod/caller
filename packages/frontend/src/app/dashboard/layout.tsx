...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      {/* Logo */}
      <div className="px-5 h-16 flex items-center gap-3 border-b border-[#1e293b]">
        <div className="w-8 h-8 bg-[#6366f1] rounded-lg flex items-center justify-center shadow-lg shadow-[#6366f1]/30">
        ...
        <div>
          ...
          <div className="text-[#475569] text-[10px] truncate max-w-[140px]">{workspace?.name ?? 'Workspace'}</div>
        </div>
      </div>

      {/* Nav */}
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <div className="px-3 mb-3 text-[10px] font-semibold text-[#475569] uppercase tracking-widest">Platform</div>
        {navItems.map(item => {
      ...
      <div className="px-3 py-4 border-t border-[#1e293b]">
        <div className="flex items-center gap-3 px-3 py-2 rounded-lg group hover:bg-[#1e293b] cursor-pointer transition-colors" onClick={logout}>
          <div className="w-8 h-8 bg-[#6366f1]/20 rounded-full flex items-center justify-center shrink-0">
            <span className="text-[#818cf8] text-xs font-bold">{initials}</span>
          </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f8fafc]">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-[240px] bg-[#0f172a] flex flex-col border-r border-[#1e293b]
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <header className="h-16 bg-white border-b border-[#e2e8f0] px-4 md:px-8 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            {/* Hamburger — mobile only */}
            <button
              onClick={() => setSidebarOpen(true)}
              className="p-2 -ml-2 rounded-lg hover:bg-[#f1f5f9] md:hidden"
