  ...
  return (
    <div className="bg-[var(--th-card)] rounded-xl border border-[var(--th-border)] p-5 shadow-[0_1px_3px_var(--th-shadow)]">
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-medium text-[var(--th-text-secondary)]">{label}</span>
        <div className={`w-8 h-8 rounded-lg ${color} flex items-center justify-center`} />
      </div>
      <div className="text-2xl font-bold text-[var(--th-text)]">{value}</div>
      {sub && <div className="text-xs text-[var(--th-text-muted)] mt-1">{sub}</div>}
    </div>
  );
}
...
function CallsWeekChart({ calls, t }: { calls: RecentCall[]; t: (key: string, vars?: Record<string, string>) => string }) {
  ...
  return (
    <div className="bg-[var(--th-card)] rounded-xl border border-[var(--th-border)] p-6 shadow-[0_1px_3px_var(--th-shadow)]">
      <h2 className="text-sm font-semibold text-[var(--th-text)] mb-5">{t('dashboard.callsThisWeek')}</h2>
        ...
        {chartData.map(day => {
          ...
          return (
            <div key={day.date} className="flex-1 flex flex-col items-center gap-1.5">
              <span className="text-xs font-semibold text-[var(--th-text)]">{day.count}</span>
              <div className="w-full flex justify-center">
                <div
                  className="w-8 rounded-t-md transition-all"
                  style={{
                    height: barHeight,
                    backgroundColor: day.count > 0 ? 'var(--th-primary)' : 'var(--th-border)',
                    minHeight: 4,
                  }}
                />
              </div>
              <span className="text-[11px] text-[var(--th-text-muted)] font-medium">{day.label}</span>
            </div>
          );
        })}
      </div>
...
export default function OverviewPage() {
  ...
  return (
    <div className="space-y-7">
      {/* Welcome */}
      <div>
        <h2 className="text-xl font-bold text-[var(--th-text)]">{t('dashboard.greeting', { timeOfDay: t(`time.${getTimeOfDay()}`), name: workspace?.name ?? 'there' })} 👋</h2>
        <p className="text-sm text-[var(--th-text-muted)] mt-1">{t('dashboard.subtitle')}</p>
      </div>

      {/* Setup Banner */}
      {showSetupBanner && (
        <div className="flex items-center justify-between px-5 py-4 bg-[var(--th-primary-bg)] border border-[#c7d2fe] rounded-xl">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-[var(--th-primary)] rounded-lg flex items-center justify-center shrink-0">
          ...
          <Link
            href="/onboarding"
            className="px-4 py-2 bg-[var(--th-primary)] hover:bg-[var(--th-primary-hover)] text-white text-xs font-semibold rounded-lg transition-all active:scale-[.98] shrink-0"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <KpiCard label={t('dashboard.totalCalls')} value={String(totalCalls)} sub={t('dashboard.allTime')} color="bg-[var(--th-primary-bg)]" />
        <KpiCard label={t('dashboard.activeNow')} value={String(activeCalls)} sub={t('dashboard.inProgress')} color="bg-[var(--th-success-bg-strong)]" />
        <KpiCard label={t('dashboard.minutesUsed')} value={String(totalMinutes)} sub={t('dashboard.thisSession')} color="bg-[var(--th-warning-bg)]" />
      ...
      <div className="bg-[var(--th-card)] rounded-xl border border-[var(--th-border)] overflow-hidden shadow-[0_1px_3px_var(--th-shadow)]">
        <div className="px-6 py-4 border-b border-[var(--th-border)] flex items-center justify-between">
          <h2 className="text-sm font-semibold text-[var(--th-text)]">{t('dashboard.recentCalls')}</h2>
          <a href="/dashboard/calls" className="text-xs text-[var(--th-primary-text)] hover:text-[var(--th-primary-hover)] font-medium">{t('dashboard.viewAll')} →</a>
        </div>
        ...
        {error ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-[var(--th-text-muted)] mt-1 max-w-xs">{t('dashboard.noCallsDesc')}</p>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-[var(--th-border-light)]">
              {calls.map(call => (
                <tr key={call.id} className="hover:bg-[var(--th-table-row-hover)] transition-colors">
                  <td className="px-6 py-3.5 text-sm font-medium text-[var(--th-text)]">
                    {call.direction === 'outbound' ? call.phone_number_to : call.phone_number_from}
                  </td>
                  <td className="px-6 py-3.5">
                    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium ${call.direction === 'outbound' ? 'bg-[var(--th-primary-bg)] text-[var(--th-primary-text)]' : 'bg-[var(--th-success-bg)] text-[var(--th-success-dark)]'}`}>
                  ...
                  <td className="px-6 py-3.5 text-sm text-[var(--th-text-secondary)]">{fmtDuration(call.duration_seconds)}</td>
                  <td className="px-6 py-3.5 text-sm text-[var(--th-text-muted)]">{fmtDate(call.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
