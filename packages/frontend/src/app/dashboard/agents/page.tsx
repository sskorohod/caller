    ...
    <div className="space-y-5">
      ...
      {loadError ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-[var(--th-card)] rounded-xl border border-[var(--th-border)] p-6 animate-pulse space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[var(--th-skeleton)] rounded-full" />
                <div className="space-y-2 flex-1">
                  <div className="h-4 bg-[var(--th-skeleton)] rounded w-1/3" />
                  <div className="h-3 bg-[var(--th-skeleton)] rounded w-1/4" />
                </div>
              </div>
              <div className="h-3 bg-[var(--th-skeleton)] rounded w-2/3" />
        ...
        <div className="bg-[var(--th-card)] rounded-xl border border-[var(--th-border)] flex flex-col items-center justify-center py-20">
          ...
          <Link href="/dashboard/agents/new" className="px-4 py-2 bg-[var(--th-primary)] text-white text-sm font-medium rounded-lg hover:bg-[var(--th-primary-hover)] transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {agents.map(agent => (
            <div key={agent.id} className="bg-[var(--th-card)] rounded-xl border border-[var(--th-border)] p-6 hover:border-[var(--th-primary-muted)] transition-all group">
              ...
              <div className="flex items-start gap-4 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-[var(--th-text)] text-base truncate">{agent.display_name || agent.name}</h3>
                    <span className={`w-2 h-2 rounded-full shrink-0 ${agent.is_active ? 'bg-[var(--th-success-icon)]' : 'bg-[var(--th-text-muted)]'}`} />
                  </div>
              ...
              {/* Description */}
              {agent.description && (
                <p className="text-sm text-[var(--th-text-secondary)] line-clamp-2 mb-3">{agent.description}</p>
              )}
              ...
              {(agent.skill_packs?.length ?? 0) > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {agent.skill_packs!.slice(0, 5).map(sp => (
                    <span key={sp.id} className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-[var(--th-warning-bg)] text-[var(--th-warning-text)] font-medium">
                  ...
                  {(agent.skill_packs?.length ?? 0) > 5 && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--th-surface)] text-[var(--th-text-muted)] font-medium">
              ...
              {/* Provider info */}
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--th-text-muted)] mb-4">
              ...
              {/* Actions */}
              <div className="flex items-center justify-between pt-3 border-t border-[var(--th-border-light)]">
                <Link
                  href={`/dashboard/agents/${agent.id}/edit`}
                  className="text-sm font-medium text-[var(--th-primary-text)] hover:text-[var(--th-primary-hover)] transition-colors flex items-center gap-1"
                >
                  {t('common.edit')}
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-red-50 text-[var(--th-text-muted)] hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
                  aria-label="Delete"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-[var(--th-overlay)] backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDeleteTarget(null)}>
