  ...
  return (
    <div className="px-3 py-4 md:p-6 space-y-4 md:space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <button onClick={() => setShowForm(true)} className="px-3 md:px-4 py-2 min-h-[44px] md:min-h-0 rounded-xl text-sm font-bold whitespace-nowrap" style={{ background: '#adc6ff', color: '#002e6a' }}>+ Create</button>
      </div>
          ...
          {codes.map(c => (
            <div key={c.id} className="glass-panel rounded-xl p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="px-2 py-0.5 rounded text-xs font-bold shrink-0" style={c.active ? { background: 'rgba(74,222,128,0.1)', color: '#4ade80' } : { background: 'rgba(107,114,128,0.1)', color: '#6b7280' }}>
                  {c.active ? 'Active' : 'Inactive'}
                </span>
              </div>
              <div className="flex items-center justify-between text-xs" style={{ color: '#c2c6d6' }}>
              ...
              <button onClick={() => handleToggle(c.id, c.active)} className="text-xs min-h-[44px] w-full rounded-lg" style={{ color: '#adc6ff', background: 'rgba(173,198,255,0.05)' }}>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end md:items-center justify-center z-50 p-0 md:p-4" onClick={() => setShowForm(false)}>
          <div className="glass-panel rounded-t-2xl md:rounded-2xl p-5 md:p-6 w-full md:max-w-md" style={{ background: '#1a202c' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} placeholder="WELCOME5"
                  className="w-full px-3 py-2 min-h-[44px] md:min-h-0 rounded-xl text-sm font-mono" style={{ background: '#2f3542', color: '#dde2f3', border: 'none', outline: 'none' }} />
              </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 min-h-[44px] md:min-h-0 text-sm" style={{ color: '#c2c6d6' }}>Cancel</button>
              <button onClick={handleCreate} disabled={saving || !form.code} className="px-4 py-2 min-h-[44px] md:min-h-0 rounded-xl text-sm font-bold disabled:opacity-50" style={{ background: '#adc6ff', color: '#002e6a' }}>
