...
type Mode = 'bidirectional' | 'unidirectional' | 'stealth';
...
export default function LiveTranslatePage() {
  ...
  useEffect(() => {
    ...
    // Backend reports the real mode + language pair for this call so the page
    // renders correctly (stealth hides voice controls).
    s.on('translator:config', (data: { mode?: Mode; my_language?: string; target_language?: string }) => {
      if (data.mode) setMode(data.mode);
      if (data.my_language) setMyLang(data.my_language);
      if (data.target_language) setTargetLang(data.target_language);
      // Stealth: re-assert the real pair in case the connect-time push overrode it.
      if (data.mode === 'stealth' && data.my_language && data.target_language) {
        s.emit('translator:set-languages', { my_language: data.my_language, target_language: data.target_language });
      }
    });
