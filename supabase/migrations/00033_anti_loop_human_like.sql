-- Add anti-loop rule to the Human-Like skill. Symptom in the last test call:
-- Manuk said "Что-что?" (audio issue), agent re-asked "Подходит?", Manuk said
-- "Ты записываешь Славу?", agent re-asked "Всё верно?" — three asks in a row
-- for one already-given agreement. We need the agent to stop after the second
-- attempt and just STATE the outcome confidently.
--
-- Idempotent: replaces the section, only updates the seeded skill.
