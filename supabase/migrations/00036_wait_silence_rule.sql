-- Add WAIT-SILENCE + MINIMAL-QUESTIONS rules to Human-Like skill.
-- Symptom from last call: caller said "Подождите секунду, дайте подумать"
-- and the agent replied "Конечно, жду. Есть слот ближе к четырём?" — it
-- acknowledged correctly, then immediately stacked a follow-up question while
-- the caller was thinking. That's robotic and pushy. Fix: when the caller
-- asks for a moment, the ONLY correct response is a 3-word acknowledgment
-- followed by silence. No new question. Also generalize: don't ask extra
-- questions, only clarify when truly ambiguous.
