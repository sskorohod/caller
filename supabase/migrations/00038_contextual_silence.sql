-- Add CONTEXTUAL SILENCE rule: agent should know when to NOT speak, even
-- without being explicitly told to wait. This is the difference between a
-- bot that fills every gap and a human who knows when silence is the right
-- response. Specifically: after delivering price/date/name, after a closing
-- question, when the caller is mid-thought, when they say "хм/мм", after they
-- said something emotional, after "я подумаю". Idempotent UPDATE.
